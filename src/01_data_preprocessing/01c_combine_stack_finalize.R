# 01c_combine_stack_finalize.R
# ============================================================
# Purpose:
# 1. Stack 2022 and 2022-24 math wide files and harmonize columns.
...
# 6. Drop uninformative rows missing all demographic data.
# 7. Run final missingness diagnostics, create descriptive summaries, 
#    item-level sample-size tables, difficulty tables, and save everything.
# ============================================================
...
file_2022 <- file.path(
  ...
  "math_umgc1ua2_anSamp2_wide.rds"
)

file_2022_24 <- file.path(
  project_dir,
  "math_v2_ua23umgc-2022-24_qa_outputs",
  "math_ua23umgc_wide.rds"
)

output_dir <- file.path(project_dir, "math_v2_umgc_ua_22_24_combined_outputs")
...
math_v2_umgc_ua_2waves4sets_raw <- bind_rows(wide_2022, wide_2022_24)

# Initial Missingness Diagnostics
miss_raw <- run_missingness_diagnostics(
  df = math_v2_umgc_ua_2waves4sets_raw,
...
dat_check <- math_v2_umgc_ua_2waves4sets_raw
...
# ------------------------------------------------------------
# Section 5. Finalize Cleaned Dataset & Apply Filters
# ------------------------------------------------------------

umgc_ids_to_replace <- umgc_rows_patched %>% distinct(global_id)

# 5A. Apply patches and deduplicate
math_v2_umgc_ua_22_24 <- math_v2_umgc_ua_2waves4sets_raw %>%
...
n_before_demo_drop <- nrow(math_v2_umgc_ua_22_24)

# 5B. Drop rows missing all demographics (except military)
rows_missing_all_demo_except_military <- math_v2_umgc_ua_22_24 %>%
  mutate(
    gender_chr = trimws(as.character(gender)),
    ethnicity_chr = trimws(as.character(ethnicity)),
    pell_chr = trimws(as.character(pell))
  ) %>%
  filter(
    is.na(age) &
      (is.na(gender_chr) | gender_chr %in% c("", "NA", "<NA>", "NULL")) &
      (is.na(ethnicity_chr) | ethnicity_chr %in% c("", "NA", "<NA>", "NULL")) &
      (is.na(pell_chr) | pell_chr %in% c("", "NA", "<NA>", "NULL")) &
      is.na(transfer)
  ) %>%
  select(global_id)

math_v2_umgc_ua_22_24 <- math_v2_umgc_ua_22_24 %>%
  anti_join(rows_missing_all_demo_except_military, by = "global_id")

# QA Checks
stopifnot(sum(duplicated(math_v2_umgc_ua_22_24$global_id)) == 0)

n_input <- nrow(math_v2_umgc_ua_2waves4sets_raw)
n_removed_umgc1 <- nrow(rows_to_remove)
n_replaced_umgc <- nrow(umgc_ids_to_replace)
n_removed_all_demo_missing <- nrow(rows_missing_all_demo_except_military)
n_final <- nrow(math_v2_umgc_ua_22_24)

cat("\nFinalize step summary:\n")
cat("Input raw rows:                             ", n_input, "\n")
cat("UMGC1 duplicate rows removed:               ", n_removed_umgc1, "\n")
cat("Original UMGC rows replaced by patches:     ", n_replaced_umgc, "\n")
cat("Rows removed (missing all demos but military):", n_removed_all_demo_missing, "\n")
cat("Final rows:                                 ", n_final, "\n")

miss_nodup <- run_missingness_diagnostics(
  df = math_v2_umgc_ua_22_24,
...
num_descriptives <- math_v2_umgc_ua_22_24 %>%
  summarise(
    age_n = sum(!is.na(age)),
...
p_age_hist <- ggplot(math_v2_umgc_ua_22_24, aes(x = age)) +
  geom_histogram(bins = 30) + labs(title = "Age Distribution") + theme_minimal()

p_transfer_hist <- ggplot(math_v2_umgc_ua_22_24, aes(x = transfer)) +
  geom_histogram(bins = 30) + labs(title = "Transferred Credits Distribution") + theme_minimal()

p_age_box <- ggplot(math_v2_umgc_ua_22_24, aes(y = age)) +
  geom_boxplot() + labs(title = "Boxplot of Age", y = "Age") + theme_minimal()

p_transfer_box <- ggplot(math_v2_umgc_ua_22_24, aes(y = transfer)) +
  geom_boxplot() + labs(title = "Boxplot of Transferred Credits") + theme_minimal()

# Item-level sample-size summary
sample_size_report <- make_item_sample_size_report(math_v2_umgc_ua_22_24)

# Math difficulty summaries
math_v2_item_counts <- qa_item_counts(math_v2_umgc_ua_22_24)
math_v2_counts_by_difficulty <- qa_response_counts_by_difficulty(math_v2_item_counts) %>%
  mutate(
    difficulty = dplyr::recode(
      assigned_difficulty,
      "EASY"   = "Easy",
      "MEDIUM" = "Medium",
      "HARD"   = "Hard",
      "HARD_P" = "Hard_precalculus"
    )
  ) %>%
  select(difficulty, items, min_count, median_count, max_count) %>%
  mutate(difficulty = factor(difficulty, levels = c("Easy", "Medium", "Hard", "Hard_precalculus"))) %>%
  arrange(difficulty)
...
qa_summary <- list(
  ...
  n_removed_all_demo_missing = n_removed_all_demo_missing,
  n_final = n_final,
...
# Data files
save_both(math_v2_umgc_ua_2waves4sets_raw, output_dir, "math_v2_umgc_ua_2waves4sets_raw")
save_both(math_v2_umgc_ua_22_24, output_dir, "math_v2_umgc_ua_22_24")

# Duplicate tracking & Patches
...
write.csv(rows_missing_all_demo_except_military, file.path(output_dir, "rows_missing_all_demo_except_military.csv"), row.names = FALSE)

# Descriptives
write.csv(cat_descriptives_df, file.path(output_dir, "categorical_descriptives_math_v2_umgc_ua_22_24.csv"), row.names = FALSE)
write.csv(num_descriptives, file.path(output_dir, "numeric_descriptives_math_v2_umgc_ua_22_24.csv"), row.names = FALSE)
...
write.csv(math_v2_counts_by_difficulty, file.path(output_dir, "math_v2_counts_by_difficulty.csv"), row.names = FALSE)
